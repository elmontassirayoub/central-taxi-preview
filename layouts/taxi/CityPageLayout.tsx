...
const CityPageLayout = ({ content }: { content: CityPageContent }) => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
                    ...
                    <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
                        <div className="space-y-6 lg:space-y-8 order-2 lg:order-1">
                            <div className="space-y-3 lg:space-y-4">
                                ...
                                <p className="text-lg sm:text-xl lg:text-2xl text-blue-100 font-light">{content.heroSubtitle}</p>
                            </div>
                            <p className="text-base sm:text-lg text-blue-50 leading-relaxed">{content.intro}</p>
                            <HeroTaxiCTA />
                        </div>
                        <div className="relative order-1 lg:order-2">
                            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-6 sm:p-8 border border-white/20">
                                <div className="text-center space-y-3 sm:space-y-4">
                                    ...
                                    <p className="text-sm sm:text-base text-blue-100">{lang === "fr" ? "Disponible tous les jours" : "Available every day"}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    {activeTab === 0 && (
                        <div className="space-y-8 sm:space-y-12">
                            ...
                            <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-2xl p-4 sm:p-6 lg:p-8">
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                                    {[content.availability, content.station, content.airport, content.business, content.night, content.events, content.tourism, content.longDistance].map((sec, idx) => (
                                        <div key={idx} className="bg-white rounded-lg p-5 text-center shadow-sm hover:shadow-md transition-shadow">
                                            <h3 className="text-base sm:text-lg font-bold text-gray-900 mb-2">{sec.title}</h3>
                                            <p className="text-gray-700 text-sm sm:text-base leading-relaxed">{sec.description}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    {activeTab === 2 && (
                        <div className="space-y-8 sm:space-y-12">
                            <div className="text-center space-y-3 sm:space-y-4">
                                ...
                                <p className="text-base sm:text-lg text-gray-600 px-4">
                                    {lang === "fr" ? "Découvrez tous nos avantages" : "Discover all our benefits"}
                                </p>
                            </div>

                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 lg:gap-8">
                                {[content.availability, content.station, content.airport, content.business, content.night, content.events, content.tourism, content.longDistance].map((benefit, idx) => (
            ...
            {/* CTA Section */}
            <section className="bg-gradient-to-r from-blue-900 to-indigo-900 text-white py-12 sm:py-16">
