...
const StationPageLayout: React.FC<StationPageLayoutProps> = ({ heroCardImageSrc, heroCardImageAlt }) => {
    const { lang } = useAppContext()
    const pageData: TaxiPageContent = Data[lang]["gare"]
                                        ...
                                        <div className="relative w-full overflow-hidden rounded-2xl border border-white/15 bg-white/5">
                                            <Image
                                                ...
                                                alt={
                                                    heroCardImageAlt ||
                                                    (lang === "fr"
                                                        ? "Taxi gare — transfert depuis les gares"
                                                        : "Station taxi — transfers from train stations")
                                                }
                                                width={900}
                                                height={600}
                ...
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        ...
                        <div className="space-y-8 sm:space-y-12">
                            <div className="text-center space-y-3 sm:space-y-4">
                                ...
                                <p className="text-base sm:text-lg text-gray-600 max-w-3xl mx-auto px-4">
                                    {lang === "fr" ? "Découvrez nos services de transfert gare professionnels" : "Discover our professional station transfer services"}
                                </p>
                            </div>
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
                                    {pageData.sections[2]?.items.map((attraction, idx) => (
                                        <div key={idx} className="bg-white rounded-lg p-3 sm:p-4 text-center shadow-sm hover:shadow-md transition-shadow">
                                            ...
                                            <p className="text-xs sm:text-sm font-medium text-gray-900">{attraction.title}</p>
                                        </div>
                                    ))}
                                </div>
                            </div>
                    ...
                    {activeTab === 3 && (
                        <div className="space-y-6 sm:space-y-8">
                            ...
                            <div className="sm:hidden space-y-4">
                                {pageData.sections[3]?.items.map((route, idx) => {
                    ...
                    {activeTab === 4 && (
                        <div className="space-y-6 sm:space-y-8">
                            ...
                            <div className="space-y-3 sm:space-y-4">
                                {pageData.sections[5]?.items.map((faq, idx) => (
