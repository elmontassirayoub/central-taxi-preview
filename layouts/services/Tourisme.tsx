import Data, { TourismeListOptionType, TourismeType } from "@/assets/data/tourisme"
import Book from "@/components/common/Buttons/Book"
import Call from "@/components/common/Buttons/Call"
import { useAppContext } from "@/context/appContext"
import Image from "next/image"

const TourismeLayout = () => {

    const { lang } = useAppContext()
    const pageData: TourismeType = Data[lang]
    ...
    return (
        <div className="">
            ...
            <div className="lg:mx-[15%] mx-5 flex flex-col lg:gap-10 gap-4 pb-10">
                <p className="text-center text-base font-medium">{pageData?.p1}</p>
                <div className="flex flex-col gap-2">
                    ...
                    <ul className="text-[#000]/[85%] text-sm grid lg:grid-cols-2 gap-3">
                        {pageData?.list1?.options?.map((op: TourismeListOptionType, idx: number) => <li className="p-3 rounded-[8px] border-[1px] border-[#000]/[50%] flex flex-col gap-2" key={idx}>
                            <p className="text-base font-semibold text-center">{op?.title}</p>
                            <span className="text-xs text-[#000]/[50%]">{op?.description}</span>
                        </li>)}
                    </ul>
                </div>
                <div className="flex flex-col gap-2">
                    <p className="text-center text-base font-medium">{pageData?.list2?.title}</p>
                    <ul className="text-[#000]/[85%] text-sm grid lg:grid-cols-2 gap-3">
                        {pageData?.list2?.options?.map((op: TourismeListOptionType, idx: number) => <li className="p-3 rounded-[8px] border-[1px] border-[#000]/[50%] flex flex-col gap-2" key={idx}>
                            <p className="text-center text-base font-semibold">{op?.title}</p>
                            <span className="text-xs text-[#000]/[50%]">{op?.description}</span>
                        </li>)}
                    </ul>
                </div>
